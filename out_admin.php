...
<head>
 ...
 button[type='bu'] {
    ...
    <script>
        function request111(sku, s2, product, id) {
            if (s2 == 1) {
    ...
    <form method="get">
        <main class="table" id="customers_table" style="margin-top: 90px;">
            <section class="table__header">
                ...
                <h1>Outbound</h1>
            <div class="input-group" style="margin-right:530px;">
            ...
            <section class="table__body">
                <table>
                    <thead>
                        <tr>
                            <?php if ($selected_status == 'admin' or $selected_status == 'manager') { ?>
                                <th>Sku</th>
                                <th>User ID</th>
                                <th>Product</th>
                                <th>Block</th>
                            ...
                            <?php } elseif ($selected_status == 'user') { ?>
                                <th>Sku</th>
                                <th>Product</th>
                                <th>Block</th>
                                <th>Inbound Date</th>
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                if ($selected_status == 'admin' or $selected_status == 'manager') {
                                    ...
                                    if($s2 == 0){
                                        echo "<td><button type='button' name = 'pay' onclick='request111(\"" . $row["Sku"] . "\", \"" . $s2 . "\", \"" . $row["ProductID"] . "\", \"" . $row["BlockID"] . "\")'>Request OutBound</button></td>";
                                    } else {
                                        echo "<td>Outbound Requested</td>";
...
<script>
        ...
        function searchTable() {
            ...
            table_rows.forEach((row, i) => {
                ...
                const product_name_datas = row.querySelector('td:nth-child(3)').textContent.toLowerCase();
                const isMatch = sku_data.includes(search_data) || product_name_data.includes(search_data) ||product_name_datas.includes(search_data);

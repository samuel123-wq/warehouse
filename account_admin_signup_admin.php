...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt = $sambungan->prepare($exist)) {
        ...
        $mail->Body = 'Your account has been registered by the Admin. Here is your ID for login : ' . $AdminID;
...
<body >
 ...
 <div class="form-containers" name="form-containers">
     <div class="title-container">
<button type='bu' onclick='event.preventDefault(); customBack3();'><img src='back.png' style='width: 25px; height: 25px;'></button>
...
function signup1(name, phone, email, password, confirmpass) {
            $.ajax({
                url: 'http://localhost/warehouse/a_signup?',
                    method: 'POST',
                data: {
                        Fullname: name,
                        Phone_Number: phone,
...
function success() {
    Swal.fire({
        title: "Successful!",
        text: "Sign Up Successfully! AdminID Has Sent To Your Email!",
        icon: "success",
        showConfirmButton: true 
    }).then((result) => {
        if (result.isConfirmed) {
            window.location='http://localhost/warehouse/a_signup';
        }
    });
}
